...
*/
function renderPaletteRow(color, element) {
  let newDiv = document.createElement('div');
  let width = getElementWidth(element);
  let colorBoxSize = width / color.length;
  for (let color of color) {
    let colorBox = createColorBox(color, colorBoxSize);
...
function renderPaletteTable() {
  const keyset = Object.keys(COLORS_9);
  const main = document.querySelector('main');
  keyset.forEach(function (color) {
    renderPaletteRow(COLORS_9[color], main);
  });
}
renderPaletteTable();
